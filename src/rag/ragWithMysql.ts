...
import { ChatHistoryManager } from "../utils/db";

dotenv.config();

async function main() {
  // สร้าง ChatHistoryManager instance
  const chatHistoryManager = new ChatHistoryManager();

  // กำหนด LINE User ID (ในการใช้งานจริงจะได้มาจาก LINE webhook)
  const lineUserId = "test-user-123";
  ...
  // โหลด chat history จากฐานข้อมูล
  let chatHistory: BaseMessage[] = await chatHistoryManager.getChatHistory(
    lineUserId
  );
  console.log(
    `Loaded ${chatHistory.length} messages from database for user: ${lineUserId}`
  );
  ...
  const chatHandle: ChatHandler = async (question: string) => {
    ...
    return {
      ...
      answerCallBack: async (answerText: string) => {
        ...
        const aiMessage = new AIMessage(answerText);
        ...
        // บันทึกลงฐานข้อมูล
        try {
          await chatHistoryManager.addMessage(lineUserId, humanMessage);
          await chatHistoryManager.addMessage(lineUserId, aiMessage);
